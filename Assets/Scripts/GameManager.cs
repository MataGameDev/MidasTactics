...
public class GameManager : MonoBehaviour
{
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (TMS.selectedUnit != null && TMS.selectedUnit.GetComponent<Units>().getMovementStateEnum(1) == TMS.selectedUnit.GetComponent<Units>().unitMoveState)
            {
                ...
                if (TMS.selectedUnitMoveRange.Contains(TMS.graph[cursorX, cursorY]))
                {
                    ...
                    else if(cursorX == TMS.selectedUnit.GetComponent<Units>().x && cursorY == TMS.selectedUnit.GetComponent<Units>().y)
                    {
    ...
    public void resetUnitsMovements(GameObject teamToReset)
    {
        ...
        {
            unit.GetComponent<Units>().moveAgain();
        }
    }
    ...
    public void cursorUIUpdate()
    {
       //If we are mousing over a tile, highlight it
        ...
        else if (hit.transform.CompareTag("Unit"))
        {
            if (tileBeingDisplayed == null)
            {
                selectedXTile = hit.transform.parent.gameObject.GetComponent<Units>().x;
                selectedYTile = hit.transform.parent.gameObject.GetComponent<Units>().y;
                cursorX = selectedXTile;
                cursorY = selectedYTile;
                TMS.quadOnMapCursor[selectedXTile, selectedYTile].GetComponent<MeshRenderer>().enabled = true;
                tileBeingDisplayed = hit.transform.parent.gameObject.GetComponent<Units>().tileBeingOccupied;

            }
            else if (tileBeingDisplayed != hit.transform.gameObject)
            {
                if (hit.transform.parent.gameObject.GetComponent<Units>().movementQueue.Count == 0)
                {
                    selectedXTile = tileBeingDisplayed.GetComponent<ClickableTile>().tileX;
                    selectedYTile = tileBeingDisplayed.GetComponent<ClickableTile>().tileY;
                    TMS.quadOnMapCursor[selectedXTile, selectedYTile].GetComponent<MeshRenderer>().enabled = false;
                    ...
                    cursorY = selectedYTile;
                    TMS.quadOnMapCursor[selectedXTile, selectedYTile].GetComponent<MeshRenderer>().enabled = true;
                    tileBeingDisplayed = hit.transform.parent.GetComponent<Units>().tileBeingOccupied;
    ...
    public void unitUIUpdate()
    {
        if (!displayingUnitInfo)
        {
            if (hit.transform.CompareTag("Unit"))
            {
                ...
                var highlightedUnits = hit.transform.parent.gameObject.GetComponent<Units>();

                UIunitCurrentHealth.SetText(highlightedUnits.currentHealthPoints.ToString());
                UIunitAttackDamage.SetText(highlightedUnits.attackDamage.ToString());
                UIunitAttackRange.SetText(highlightedUnits.attackRange.ToString());
                UIunitMoveSpeed.SetText(highlightedUnits.moveSpeed.ToString());
                UIunitName.SetText(highlightedUnits.unitName);
                UIunitSprite.sprite = highlightedUnits.unitSprite;
            ...
            else if (hit.transform.CompareTag("Tile"))
            {
                if (hit.transform.GetComponent<ClickableTile>().unitOnTile != null)
                {
                    ...
                    UIunitCanvas.enabled = true;
                    displayingUnitInfo = true;
                    var highlightedUnits = unitBeingDisplayed.GetComponent<Units>();
    ...
    public void teamHealthbarColorUpdate()
    {
        for(int i = 0; i < numberOfTeams; i++)
        {
            ...
            if(team == returnTeam(currentTeam))
            {
                ...
                {
                    unit.GetComponent<Units>().changeHealthBarColour(0);
                }
            }
            else
            {
                foreach (Transform unit in team.transform)
                {
                    unit.GetComponent<Units>().changeHealthBarColour(1);
                }
            }
        }
    ...
    public List<Node> generateCursorRouteTo(int x, int y)
    {
        ...
        if (TMS.selectedUnit.GetComponent<Units>().x == x && TMS.selectedUnit.GetComponent<Units>().y == y)
        {
        ...
        //TMS.selectedUnit.GetComponent<Units>().path = null;
        currentPathForUnitRoute = null;
        //from wiki dijkstra's
        Dictionary<Node, float> dist = new Dictionary<Node, float>();
        Dictionary<Node, Node> prev = new Dictionary<Node, Node>();
        Node source = TMS.graph[TMS.selectedUnit.GetComponent<Units>().x, TMS.selectedUnit.GetComponent<Units>().y];
