...
public class BattleManager : MonoBehaviour
{
   ...
   //This is used to check if the battle has been finished
    ...
    {
        battleStatus = true;
        var initiatorUnit = initiator.GetComponent<Units>();
        var recipientUnit = recipient.GetComponent<Units>();
        ...
        {
            GameObject tempParticle = Instantiate( recipientUnit.GetComponent<Units>().damagedParticle,recipient.transform.position, recipient.transform.rotation);
    ...
    {
        if (unitToCheck.GetComponent<Units>().currentHealthPoints <= 0)
        {
            return true;
        }
        return false;
    ...
    public IEnumerator attack(GameObject unit, GameObject enemy)
    {
        ...
        unit.GetComponent<Units>().setWalkingAnimation();
        ...
        while (battleStatus)
        {
            ...
            StartCoroutine(CSS.camShake(.2f,unit.GetComponent<Units>().attackDamage,getDirection(unit,enemy)));
            if(unit.GetComponent<Units>().attackRange == enemy.GetComponent<Units>().attackRange && enemy.GetComponent<Units>().currentHealthPoints - unit.GetComponent<Units>().attackDamage > 0)
            {
                StartCoroutine(unit.GetComponent<Units>().displayDamageEnum(enemy.GetComponent<Units>().attackDamage));
                StartCoroutine(enemy.GetComponent<Units>().displayDamageEnum(unit.GetComponent<Units>().attackDamage));
            }
            ...
            //unit.GetComponent<Units>().displayDamage(enemy.GetComponent<Units>().attackDamage);
            //enemy.GetComponent<Units>().displayDamage(unit.GetComponent<Units>().attackDamage);
    ...
    public IEnumerator returnAfterAttack(GameObject unit, Vector3 endPoint) {
        ...
        unit.GetComponent<Units>().setWaitIdleAnimation();
        unit.GetComponent<Units>().wait();
