...
public class TileMap : MonoBehaviour    
{
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            //Move the unit
            else if (selectedUnit.GetComponent<Units>().unitMoveState == selectedUnit.GetComponent<Units>().getMovementStateEnum(1) && selectedUnit.GetComponent<Units>().movementQueue.Count == 0)
            {
                ...
                if ( selectTileToMoveTo())
                {
                    ...
                    unitSelectedPreviousX = selectedUnit.GetComponent<Units>().x;
                    unitSelectedPreviousY = selectedUnit.GetComponent<Units>().y;
                    previousOccupiedTile = selectedUnit.GetComponent<Units>().tileBeingOccupied;
                    selectedUnit.GetComponent<Units>().setWalkingAnimation();
                    moveUnit();
                    
                    StartCoroutine(moveUnitAndFinalize());
                    //The moveUnit function calls a function on the unitsm when the movement is completed the finalization is called from that script.
            ...
            else if(selectedUnit.GetComponent<Units>().unitMoveState == selectedUnit.GetComponent<Units>().getMovementStateEnum(2))
            {
                finalizeOption();
            }
        ...
        if (Input.GetMouseButtonDown(1))
        {
            if (selectedUnit != null)
            {
                if (selectedUnit.GetComponent<Units>().movementQueue.Count == 0 && selectedUnit.GetComponent<Units>().combatQueue.Count==0)
                {
                    if (selectedUnit.GetComponent<Units>().unitMoveState != selectedUnit.GetComponent<Units>().getMovementStateEnum(3))
                    {
                        //unselectedSound.Play();
                        selectedUnit.GetComponent<Units>().setIdleAnimation();
                        deselectUnit();
                    }
                }
                else if (selectedUnit.GetComponent<Units>().movementQueue.Count == 1)
                {
                    selectedUnit.GetComponent<Units>().visualMovementSpeed = 0.5f;
                }
            }
        }
    ...
    public void moveUnit()
    {
        if (selectedUnit != null)
        {
            selectedUnit.GetComponent<Units>().MoveNextTile();
        }
    }
    ...
    public void setIfTileIsOccupied()
    {
        foreach (Transform team in unitsOnBoard.transform)
        {
            ...
            foreach (Transform unitOnTeam in team) { 
                int unitX = unitOnTeam.GetComponent<Units>().x;
                int unitY = unitOnTeam.GetComponent<Units>().y;
                unitOnTeam.GetComponent<Units>().tileBeingOccupied = tilesOnMap[unitX, unitY];
    ...
    public void generatePathTo(int x, int y)
    {
        ...
        if (selectedUnit.GetComponent<Units>().x == x && selectedUnit.GetComponent<Units>().y == y){
            Debug.Log("clicked the same tile that the unit is standing on");
            currentPath = new List<Node>();
            selectedUnit.GetComponent<Units>().path = currentPath;
        ...
        selectedUnit.GetComponent<Units>().path = null;
        currentPath = null;
        //from wiki dijkstra's
        Dictionary<Node, float> dist = new Dictionary<Node, float>();
        Dictionary<Node, Node> prev = new Dictionary<Node, Node>();
        Node source = graph[selectedUnit.GetComponent<Units>().x, selectedUnit.GetComponent<Units>().y];
    ...
    public bool unitCanEnterTile(int x, int y)
    {
        ...
        {
            if (tilesOnMap[x, y].GetComponent<ClickableTile>().unitOnTile.GetComponent<Units>().teamNum != selectedUnit.GetComponent<Units>().teamNum)
            {
                return false;
            }
        }
            ...
            if (unitSelected == false)
            {
                ...
                if (hit.transform.gameObject.CompareTag("Tile"))
                {
                    if (hit.transform.GetComponent<ClickableTile>().unitOnTile != null)
                    {
                        ...
                        tempSelectedUnit = hit.transform.GetComponent<ClickableTile>().unitOnTile;
                        if (tempSelectedUnit.GetComponent<Units>().unitMoveState == tempSelectedUnit.GetComponent<Units>().getMovementStateEnum(0)
                            && tempSelectedUnit.GetComponent<Units>().teamNum == GMS.currentTeam
                            )
                        {
                            disableHighlightUnitRange();
                            selectedUnit = tempSelectedUnit;
                            selectedUnit.GetComponent<Units>().map = this;
                            selectedUnit.GetComponent<Units>().setMovementState(1);
                            unitSelected = true;
                ...
                else if (hit.transform.parent != null && hit.transform.parent.gameObject.CompareTag("Unit"))
                {
                        ...
                        )
                    {
                        ...
                        //selectedUnit.GetComponent<Units>().x = (int)selectedUnit.transform.position.x;
                        // selectedUnit.GetComponent<Units>().y = (int)selectedUnit.transform.position.z;
                        selectedUnit.GetComponent<Units>().map = this;
                        unitSelected = true;
    ...
    {
        tilesOnMap[selectedUnit.GetComponent<Units>().x, selectedUnit.GetComponent<Units>().y].GetComponent<ClickableTile>().unitOnTile = selectedUnit;
        //After a unit has been moved we will set the unitMoveState to (2) the 'Moved' state


        selectedUnit.GetComponent<Units>().setMovementState(2);
        ...
        if (unitSelected == false && GMS.tileBeingDisplayed!=null)
        {
            ...
            if (GMS.tileBeingDisplayed.GetComponent<ClickableTile>().unitOnTile != null)
            {
                               ...
                               )
                {
                    ...
                    selectedUnit.GetComponent<Units>().map = this;
                    selectedUnit.GetComponent<Units>().setMovementState(1);
                    selectedUnit.GetComponent<Units>().setSelectedAnimation();
                    unitSelected = true;
                    highlightUnitRange();
    ...
    if (Physics.Raycast(ray, out hit))
    {
        ...
        if (hit.transform.gameObject.CompareTag("Tile"))
        {
            if (hit.transform.GetComponent<ClickableTile>().unitOnTile != null)
            {
                ...
                int unitX = unitOnTile.GetComponent<Units>().x;
                int unitY = unitOnTile.GetComponent<Units>().y;

                if (unitOnTile == selectedUnit)
                {
                    disableHighlightUnitRange();
                    Debug.Log("ITS THE SAME UNIT JUST WAIT");
                    selectedUnit.GetComponent<Units>().wait();
                    selectedUnit.GetComponent<Units>().setWaitIdleAnimation();
                    selectedUnit.GetComponent<Units>().setMovementState(3);
                    deselectUnit();


                    }
                    else if (unitOnTile.GetComponent<Units>().teamNum != selectedUnit.GetComponent<Units>().teamNum && attackableTiles.Contains(graph[unitX,unitY]))
                {
                        if (unitOnTile.GetComponent<Units>().currentHealthPoints > 0)
                        {
                            Debug.Log("We clicked an enemy that should be attacked");
                            Debug.Log(selectedUnit.GetComponent<Units>().currentHealthPoints);
        ...
        else if (hit.transform.parent != null && hit.transform.parent.gameObject.CompareTag("Unit"))
        {
            ...
            int unitX = unitClicked.GetComponent<Units>().x;
            int unitY = unitClicked.GetComponent<Units>().y;
            ...
            else if (unitClicked.GetComponent<Units>().teamNum != selectedUnit.GetComponent<Units>().teamNum && attackableTiles.Contains(graph[unitX, unitY]))
            {
                    if (unitClicked.GetComponent<Units>().currentHealthPoints > 0)
                    {
                        
                        Debug.Log("We clicked an enemy that should be attacked");
                        Debug.Log("Add Code to Attack enemy");
                        //selectedUnit.GetComponent<Units>().setAttackAnimation();
    ...
    public void deselectUnit()
    {
        ...
        if (selectedUnit != null)
        {
            if (selectedUnit.GetComponent<Units>().unitMoveState == selectedUnit.GetComponent<Units>().getMovementStateEnum(1))
            {
            disableHighlightUnitRange();
            disableUnitUIRoute();
            selectedUnit.GetComponent<Units>().setMovementState(0);

       
            selectedUnit = null;
            unitSelected = false;
            }
            else if (selectedUnit.GetComponent<Units>().unitMoveState == selectedUnit.GetComponent<Units>().getMovementStateEnum(3) )
            {
                disableHighlightUnitRange();
                disableUnitUIRoute();
                unitSelected = false;
            ...
            else
            {
                disableHighlightUnitRange();
                disableUnitUIRoute();
                tilesOnMap[selectedUnit.GetComponent<Units>().x, selectedUnit.GetComponent<Units>().y].GetComponent<ClickableTile>().unitOnTile = null;
                tilesOnMap[unitSelectedPreviousX, unitSelectedPreviousY].GetComponent<ClickableTile>().unitOnTile = selectedUnit;

                selectedUnit.GetComponent<Units>().x = unitSelectedPreviousX;
                selectedUnit.GetComponent<Units>().y = unitSelectedPreviousY;
                selectedUnit.GetComponent<Units>().tileBeingOccupied = previousOccupiedTile;
    ...
    public void highlightUnitRange()
    {
        ...
        int attRange = selectedUnit.GetComponent<Units>().attackRange;
        int moveSpeed = selectedUnit.GetComponent<Units>().moveSpeed;


        Node unitInitialNode = graph[selectedUnit.GetComponent<Units>().x, selectedUnit.GetComponent<Units>().y];
        ...
        foreach (Node n in totalAttackableTiles)
        {
            ...
            if (tilesOnMap[n.x, n.y].GetComponent<ClickableTile>().unitOnTile != null)
            {
                ...
                if (unitOnCurrentlySelectedTile.GetComponent<Units>().teamNum != selectedUnit.GetComponent<Units>().teamNum)
                {
    ...
    public HashSet<Node> getUnitAttackOptionsFromPosition()
    {
        ...
        HashSet<Node> neighbourHash = new HashSet<Node>();
        HashSet<Node> seenNodes = new HashSet<Node>();
        Node initialNode = graph[selectedUnit.GetComponent<Units>().x, selectedUnit.GetComponent<Units>().y];
        int attRange = selectedUnit.GetComponent<Units>().attackRange;
    ...
    public HashSet<Node> getTileUnitIsOccupying()
    {
        ...
        int x = selectedUnit.GetComponent<Units>().x;
        int y = selectedUnit.GetComponent<Units>().y;
    ...
    public IEnumerator moveUnitAndFinalize()
    {
        disableHighlightUnitRange();
        disableUnitUIRoute();
        while (selectedUnit.GetComponent<Units>().movementQueue.Count != 0)
        {
    ...
    public IEnumerator deselectAfterMovements(GameObject unit, GameObject enemy)
    {
        ...
        while (unit.GetComponent<Units>().combatQueue.Count > 0)
        {
            yield return new WaitForEndOfFrame();
        }
        while (enemy.GetComponent<Units>().combatQueue.Count > 0)
        {
            yield return new WaitForEndOfFrame();
    ...
    public bool selectTileToMoveTo()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (hit.transform.gameObject.CompareTag("Tile")){
                ...
                //var units = selectedUnit.GetComponent<Units>();

                if (selectedUnitMoveRange.Contains(nodeToCheck)) {
                    if ((hit.transform.gameObject.GetComponent<ClickableTile>().unitOnTile == null || hit.transform.gameObject.GetComponent<ClickableTile>().unitOnTile == selectedUnit) && (selectedUnitMoveRange.Contains(nodeToCheck)))
                    {
            ...
            else if (hit.transform.gameObject.CompareTag("Unit"))
            {
                ...
                if (hit.transform.parent.GetComponent<Units>().teamNum != selectedUnit.GetComponent<Units>().teamNum)
                {
                ...
                else if(hit.transform.parent.gameObject == selectedUnit)
                {
                    ...
                    generatePathTo(selectedUnit.GetComponent<Units>().x, selectedUnit.GetComponent<Units>().y);
                    return true;
                }
            }
